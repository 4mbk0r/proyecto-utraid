    ...
    <v-card>
        <v-form ref="seleccion_municipio">
            <V-row>
                ...
                <v-col class="text-right" color="erroe" cols="6">
                    <v-btn @click="verificar_eliminar">Eliminar</v-btn>
                </v-col>
        ...
        <v-dialog v-model="dialogEliminarSala" persistent max-width="500">
            <v-card>
                <v-card-title>
                    Eliminar sala
                </v-card-title>
                <v-card-text>
                    ¿Estás seguro de que deseas eliminar la sala "{{ datos.descripcion }}"?. Esta acción no se puede revertir.
                </v-card-text>
                <v-card-actions class="align-left">
                    <v-btn color="primary" @click="dialogEliminarSala = false">Cancelar</v-btn>
                    <v-btn color="error" @click="eliminar_sala">Eliminar</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        <v-dialog v-model="dialogErrorEliminarSala" persistent max-width="500">
            <v-card>
                <v-card-title>
                    Error al eliminar sala
                </v-card-title>
                <v-card-text>
                    No se pudo eliminar la sala "{{ datos.descripcion }}" porque {{ mensaje_error }}.
                </v-card-text>
                <v-card-actions>
                    <v-btn @click="dialogErrorEliminarSala = false">Aceptar</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        <!--{{ datos }}-->
...
export default {
    ...
    methods: {
        ...
        async eliminar_sala($e) {
            //let f = this.selectedEvent.fichas.id_ficha
            var res = await axios({
                method: 'post',
                url: `/${process.env.MIX_CARPETA}/eliminar_sala2`,
                data: this.datos
            }).then(
                (response) => {
                ...
                this.mensaje_error = err.response.data.error
                this.dialogErrorEliminarSala = true
                this.dialogEliminarSala = false
        ...
        async guardar() {
            ...
            var res = await axios({
                method: 'post',
                url: `/${process.env.MIX_CARPETA}/cambiar_horario`,
                data: {
                    config_hora: this.s,
                    sala: this.datos,
                    fecha: this.fecha,
