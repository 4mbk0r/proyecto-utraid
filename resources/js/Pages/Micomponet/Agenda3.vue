    ...
    <v-card>
        ...
        <v-row>
            <v-col>
                ...
                <v-row>
                    <v-col>
                        <v-btn class="mx-2" dark color="indigo" @click="Crearnuevasala">
                            Nueva Sala
                            <v-icon dark>
                                mdi-plus
                            </v-icon>
                                ...
                                <v-col>
                                    <v-row align="center" justify="center">
                                        ...
                                        <v-col cols="12">
                                            <v-autocomplete v-model="selectequipo" :items="equipos"
                                                :item-text="item => (item.nombres + ' ' + item.ap_paterno + ' ' + item.ap_materno)"
                                                :item-value="item => item" :rules="selecionRules" cache-items class="mx-4"
                ...
                <!--<v-data-table :headers="headers" :items="items" :group-by="['category']" :group-desc="true"
                    class="elevation-1">
                    <template v-slot:group.header="{ group, items, toggle, isOpen }">
                        <th :colspan="headers.length">
                            ...
                            {{ group.toUpperCase() }}
                            <v-btn outlined @click="addNewItemToGroup(group)">
                                Nueva Sala
        ...
        <v-dialog v-model="dialog_nuevasala"  persistent :scrim="false"
            transition="dialog-bottom-transition">
            <v-card>
                <v-toolbar dark color="black">
                    <v-btn icon dark @click="dialog_nuevasala = false">
                        <v-icon>mdi-close</v-icon>
                    </v-btn>
                ...
                <v-card>
                    <v-card-text>
                        <!--------
                        esto es para el nuevo consulta    
                        {{ cita_nueva }}

                        --------->
                        
                        <crearSala v-if="dialog_nuevasala" :fecha="fecha_calendario" :datos="cita_nueva"
                        @cerrar_ventana="cerrar_nuevasala"
                        >
                        </crearSala>
                    </v-card-text>
...
import crearSala from '@/Pages/Configuracion/crearSala'
        ...
        async pedir_datos(date) {
            ...
            var res = await axios({
                ...
                (response) => {
                    ...
                    salas.sort((a, b) => a.descripcion.localeCompare(b.descripcion));
