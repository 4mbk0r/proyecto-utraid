<template>
    <v-app>
        <div>
                    ...
                    <v-col>
                        <v-sheet height="64">
                            <v-toolbar>
                                ...
                                <input id="txtDate" type="date" refs="seldate" onkeydown="return false"
                                ...
                                <v-spacer></v-spacer>
                                <button type="button" @click="ventana_modal"
            ...
            <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
                ...
                <v-card color="#1CA698">
                    <v-container>
                        ...
                        <v-btn icon color="#f2f2f2" @click="av_buscar = !av_buscar">
                    ...
                    <v-container>
                        ...
                        <v-form ref="formcita" @submit.prevent="addEvent">
                            ...
                            <v-container ma-0 pa-0>
                                <v-row no-gutters>
                                    <v-col cols="10" md="6">
                                        <v-text-field v-model="paciente.ci" type="text" pa-0 solo @change="prueba3"
                                            label="Carnet" required></v-text-field>
                                    </v-col>

                                    <v-col cols="10" md="6">
                                        <v-select v-model="paciente.expendido" pa-0 solo :items="sdepartamentos"
                                            :rules="selectRules" label="Expedido">
                                        </v-select>
                                    </v-col>
                                </v-row>
                            </v-container>

                            <v-container v-if="paciente" ma-0 pa-0>
                                <v-row outlined>
                                ...
                                <v-container v-if="informacion" ma-0 pa-0>
                                    <v-row no-gutters>
                                        <v-col cols="12" md="4">
                                            <v-text-field v-model="paciente.nombre" :rules="nombreRules" type="text"
                                                pa-0 solo label="Nombre">
                                            </v-text-field>
                                        </v-col>
                                        <v-col cols="12" md="4">
                                            <v-text-field v-model="paciente.ap_paterno" :rules="nombreRules" type="text"
                                                pa-0 solo label="Apellido Paterno">
                                            </v-text-field>
                                        </v-col>
                                        <v-col cols="12" md="4">
                                            <v-text-field v-model="paciente.ap_materno" :rules="nombreRules" type="text"
                                                pa-0 solo label="Apellido Materno">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row no-gutters>
                                        <v-col cols="10" md="6">
                                            <v-text-field v-model="paciente.correo" type="email" label="Correo" pa-0
                                                solo>
                                            </v-text-field>
                                        </v-col>

                                        <v-col cols="10" md="6" pa-0 solo>
                                            <v-text-field v-model="paciente.numero" type="text" label="Numero Celular"
                                                pa-0 solo>
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row no-gutters>
                                        <v-col cols="12" md="4">
                                            <v-text-field v-model="paciente.fecha_nacimiento" :min="minFechaNac"
                                                :max="maxFechaNac" type="date" label="Fecha de nacimiento" pa-0 solo>
                                            </v-text-field>
                                        </v-col>
                                        <v-col cols="12" md="4">
                                            <v-text-field v-model="paciente.direccion" type="text" label="Direccion"
                                                pa-0 solo>
                                            </v-text-field>
                                        </v-col>
                                        <v-col cols="12" md="4">
                                            <v-select v-model="paciente.sexo" :items="ssexo" color="purple darken-3"
                            ...
                            <v-container v-if="desserts.length >= 1">
                                ...
                                <div v-if="Historia">
                                    <v-data-table :headers="headers"
                                        :footer-props="{ itemsPerPageText: 'Pacientes por pagina' }" :items="desserts"
                                        :sort-desc.sync="sortDesc" sort-by="fecha" class="elevation-1 cyan lighten-3">
            ...
            <v-dialog v-model="v_datos_informacion" fullscreen hide-overlay transition="dialog-bottom-transition">
                <v-toolbar dark color="#1CA698">
                    <v-btn icon dark @click="v_datos_informacion=false;limipiar_form">
                        <v-icon>mdi-close</v-icon>
                    </v-btn>
                    <v-toolbar-title>Datos Personales</v-toolbar-title>
                    <v-spacer></v-spacer>
                ...
                <v-card color="#1CA698">
                    ...
                    <v-container>
                        <v-tabs v-model="datos_informacion" align-with-title>
                        ...
                        <v-tabs-items v-model="datos_informacion" touchless>
                            <v-tab-item>
                                <v-card flat>
                                    <datospersonales :paciente="paciente" :op="paciente_cat" />
                                </v-card>
                            </v-tab-item>
                            <v-tab-item>
                                <v-card flat>
                        ...
                        <v-form ref="form_cita" @submit.prevent="">
                            ...
                            <div class="text-center">
                                ...
                                <div id="main" class="grid grid-cols-3 gap-1 justify-evenly">
                                    <v-text-field v-model="fecha_cita_actual" :rules="nombreRules" :min="fechacitaMin"
                                        @change="change_fecha2" type="date" label="Fecha de Cita">
                                    </v-text-field>
                                    <div>
                                        <v-select v-model="equipo_actual" :items="equipos_actuales" :rules="selectRules"
                                            @change="cambioequipo" color="purple darken-3" label="Equipo">
                                        </v-select>
                                    </div>
                                    <div>
                                        <v-select v-model="t_equipo" :items="tiempos_actuales" :rules="selectRules"
                                            color="purple darken-3" label="Hora de inicio">
                                        </v-select>
                                    </div>
                                </div>
                                <div>

                                    <v-select v-model="ttipocita" :items="tipo_cita" color="purple darken-3"
                                        label="Tipo de cita">
                                    </v-select>
                            ...
                            <v-btn type="submit" color="primary" class="mr-4" @click="guardar_cita"
                                @click.stop="v_agendar = false">
                                Guardar Cita
                            </v-btn>
                            <v-btn type="submit" color="primary" class="mr-4" @click="imprimir_directo"
                                @click.stop="v_agendar = false">
                                Boleta
                            </v-btn>
                        </v-form>
                ...
                <v-card>
                    <v-container>
                        ...
                        <v-form ref="form_cita_edit" @submit.prevent="">
                            <div id="main" class="text-center">
                                <div class="grid grid-cols-2 gap-1 justify-evenly">
                                    ...
                                    <div class="text-black">
                                        <v-select label="Expedido" v-model="editar.nom_municipio"
                                            :items="sdepartamentos" :rules="selectRules">
                                        </v-select>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-3 gap-1 justify-evenly">
                                    <div>
                                        <v-text-field v-model="editar.fecha" :rules="nombreRules" :min="fechacitaMin"
                                            @change="change_fecha(editar)" type="date" label="Fecha de Cita">
                                        </v-text-field>
                                    </div>
                                    <div>
                                        <v-select v-model="editar.equipo" :items="equipos_actuales" :rules="selectRules"
                                            @change="cambioequipos(editar)" color="purple darken-3" label="Equipo">
                                        </v-select>
                                    </div>
                                    <div>
                                        <v-select v-model="editar.hora_inicio" :items="tiempos_actuales"
                                            :rules="selectRules" color="purple darken-3" label="Hora de inicio">
                                        </v-select>
                                    </div>
                                </div>
                                <div class="grid grid-cols-3 gap-1 justify-evenly">
                                    <div>
                                        <v-select v-model="editar.tipo_cita" :items="tipo_cita" color="purple darken-3"
                                            label="Tipo de cita">
                                        </v-select>
                                    </div>
                                    <div>
                                        <v-select v-model="editar.lugar" :items="ttlugares" color="purple darken-3"
                                            label="Lugar">
                                        </v-select>
                                    </div>
                                    <div>
                            ...
                            <v-btn type="submit" color="primary" class="mr-4" @click="guardar_cita_edit"
                                @click.stop="v_agendar = false">
                                Guardar
                            </v-btn>
            ...
            <v-dialog id="modalInvoice" v-model="v_imprimir" fullscreen transition="dialog-bottom-transition">
                ...
                <v-card>
                    ...
                    <div id="print">
                        <div class="center_columna .item1">
                            <div><img
                                    src="https://pbs.twimg.com/profile_images/1405210297036525568/0KEhDx7D_400x400.jpg"
                            ...
                            <div><img
                                    src="https://scontent.flpb1-1.fna.fbcdn.net/v/t1.6435-9/195975552_5609812179090371_6523992678396965776_n.jpg?_nc_cat=104&ccb=1-7&_nc_sid=174925&_nc_ohc=Q__ncnQ9wdsAX9F1k9j&_nc_ht=scontent.flpb1-1.fna&oh=00_AT9AKy3kTnKXSiEuySxYnEqyl5t_M59YrL0o4MECdWlANA&oe=62B0F082"
                                    width="100px" height="100px"></div>
                        </div>
                        <div class="wrapper alinear_elemento">
                            <div class="box one">
                                <label>NOMBRE COMPLETO:</label>
                                <label class="texto_nombre">{{ selectedEvent.nombre }} {{ selectedEvent.ap_paterno }}
                                    {{ selectedEvent.ap_materno }}</label>
                            </div>
                            <div class="box two">
                                <label>Fecha:</label>
                        ...
                        <div class="grid-item">
                            <div class="alinear_elemento">
                                ...
                                <textarea class="observaciones" rows="2" cols="100"
                                    v-model="selectedEvent.observacion">{{ selectedEvent.observacion }}</textarea>
                            </div>
                        </div>
                        <div>
                ...
                <v-card color="grey lighten-4" min-width="350px" flat>
                    ...
                    <v-card-text>
                        ...
                        <label class="font-bold" v-if="selectedEvent.se_presento == 'Si'">
                            Fue Atendido:
                            <v-select v-model="selectedEvent.ci_doctor" :dark="color" hint="Pick your meal"
                                item-text="nombre" item-value="ci" :items="doctores"
                                :menu-props="{ top: true, offsetY: true }" label=""></v-select>
                        </label>
                    </v-card-text>
                    <v-card-actions>
    ...
    methods: {
        ...
        async prueba3() {
            ...
            if (datos.length == 1) {
                ...
                var b = await axios.post('api/datos_citas/' + this.paciente.ci).then();
                this.desserts = b['data'];
                console.log(b);
                if (this.desserts.length == 0) {
                    this.desserts = [];
            ...
            if (this.paciente.sexo == '') {
                this.paciente.sexo = 'No se tiene registro';
            }
            if (this.paciente.expendido == '') {
                this.paciente.deparamento = 'No se tiene registro';
            }
        ...
        async prueba2(vci) {
            ...
            var a = await axios.post('api/datos_usuario/' + vci).then();
            this.datos_usuario = a['data'];
            if (this.datos_usuario.length == 0) {
                this.msg_usuario = 'Nuevo';
            }
            if (this.datos_usuario.length == 1) {
                this.msg_usuario = 'Antiguo';
                var b = await axios.post('api/datos_citas/' + vci).then();
                this.desserts = b['data'];
                console.log(b);
                if (this.desserts.length == 0) {
                    this.desserts = [];
